...
<p align="center">
  <b>Deploy do bot Anime News em VPS Linux com Docker</b><br>
...
```bash
# Do seu PC Windows:
scp -r ./anime-news-bot/* user@seu-servidor:/opt/anime-news-bot/
```
...
```
    Name              Command        State
-----------------------------------------------
anime-news-bot   python -u main.py  Up
```
...
docker-compose exec anime-bot bash

# Executar Python no container
docker-compose exec anime-bot python -c "print('Hello from container')"
...
anime-news-bot   0.5%    120MiB / 2GiB       6%
```
...
cd /opt/anime-news-bot
tar -czf ~/anime-backup-$(date +%Y%m%d-%H%M%S).tar.gz \
  config.json \
  history.json \
  state.json \
  .env
...
0 3 * * * cd /opt/anime-news-bot && tar -czf ~/backups/anime-news-$(date +\%Y\%m\%d).tar.gz config.json history.json state.json .env
```
...
tar -xzf anime-news-full-backup.tar.gz
...
<p align="center">
  ...
  <i>Desenvolvido com ❤️ para a comunidade Anime</i>
</p>
